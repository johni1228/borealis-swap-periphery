...
contract ExampleFlashSwap is IUniswapV2Callee {
    ...
    constructor(address _factory, address _factoryV1, address router) public {
        ...
        factory = _factory;
        WETH = IWETH(IBorealisswapV2Router01(router).WETH());
    }

    // needs to accept BNB from any V1 exchange and WETH. ideally this could be enforced, as in the router,
    // but it's not possible because it requires a call to the v1 factory, which takes too much gas
    receive() external payable {}

    // gets tokens/WETH via a V2 flash swap, swaps for the BNB/tokens on V1, repays V2, and keeps the rest!
    function uniswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external override {
        ...
        assert(msg.sender == BorealisswapV2Library.pairFor(factory, token0, token1)); // ensure that msg.sender is actually a V2 pair
        ...
        IBorealisswapV1Exchange exchangeV1 = IBorealisswapV1Exchange(factoryV1.getExchange(address(token))); // get V1 exchange
        ...
        if (amountToken > 0) {
            ...
            uint amountRequired = BorealisswapV2Library.getAmountsIn(factory, amountToken, path)[0];
            ...
            uint amountRequired = BorealisswapV2Library.getAmountsIn(factory, amountBNB, path)[0];
