...
import './interfaces/IBorealisswapRouter02.sol';
import './libraries/BorealisswapLibrary.sol';
import './libraries/SafeMath.sol';
import './interfaces/IERC20.sol';
import './interfaces/IWETH.sol';

contract BorealisswapRouter02 is IBorealisswapRouter02 {
    ...
    function _addLiquidity(
        ...
        if (IBorealisswapFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IBorealisswapFactory(factory).createPair(tokenA, tokenB);
        }
        (uint reserveA, uint reserveB) = BorealisswapLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = BorealisswapLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'BorealisswapRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = BorealisswapLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = IBorealisPair(pair).burn(to);
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'BorealisRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, BorealisswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'BorealisRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, BorealisswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    {
        require(path[0] == WETH, 'BorealisRouter: INVALID_PATH');
    ...
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'BorealisRouter: INVALID_PATH');
        amounts = BorealisswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'BorealisRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, BorealisswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'BorealisRouter: INVALID_PATH');
        amounts = BorealisswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'BorealisRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(BorealisswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, BorealisswapLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
            'BorealisRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'BorealisRouter: INSUFFICIENT_OUTPUT_AMOUNT');
